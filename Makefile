...
.PHONY: licenses
licenses: path go-licenses ## Generate $(path)/third-party-licenses.md, skipped when the file already exists
	@if [ -f $(path)/third-party-licenses.md ]; then \
		echo "$(path)/third-party-licenses.md exists, skip license generation"; \
	else \
		mkdir -p $(path); \
		cd cmd && $(GOLICENSES) report . --template ../build/licenses.tpl > ../tpl.md && cd ..; \
