...
func TestAppCacheRedisDsnOverwrite(t *testing.T) {
	t.Run("AppCache with DSN", func(t *testing.T) {
		...
		env := store.GetEnv()
		var foundCacheType, foundCacheURL bool
		for _, envVar := range env {
			...
			if envVar.Name == "K8S_CACHE_HOST" || envVar.Name == "K8S_CACHE_PORT" || envVar.Name == "K8S_CACHE_INDEX" {
		...
		assert.True(t, foundCacheURL, "K8S_CACHE_URL should be set with DSN value")
	})
