...
func TestStoreContainer(t *testing.T) {
	...
	assert.Equal(t, []corev1.VolumeMount{{Name: "vol1", MountPath: "/path1"}, {Name: "vol2", MountPath: "/path2"}}, baseContainer.Spec.Container.VolumeMounts)
	assert.Equal(t, []corev1.LocalObjectReference{{Name: "secret2"}}, baseContainer.Spec.Container.ImagePullSecrets)
	assert.Equal(t, []corev1.Volume{{Name: "vol1"}, {Name: "vol2"}}, baseContainer.Spec.Container.Volumes)
