...
func TestSetupJob(t *testing.T) {
		...
		expected := &batchv1.Job{
			ObjectMeta: metav1.ObjectMeta{
				...
				Labels: map[string]string{
					util.ShopwareKey("store.name"): "test-store",
					util.ShopwareKey("store.type"): "setup",
				},
				Annotations: map[string]string{
	...
	t.Run("test container merge spec", func(t *testing.T) {
		...
		assert.Equal(t, "setup-volume", container.VolumeMounts[1].Name)
		assert.Equal(t, "/setup", container.VolumeMounts[1].MountPath)
