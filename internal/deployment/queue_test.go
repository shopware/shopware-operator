...
func TestSelectStatsOutput(t *testing.T) {
	assert.Equal(t, `{"transports":{}}`, selectStatsOutput(`{"transports":{}}`, "warning"))
	assert.Equal(t, `{"transports":{}}`, selectStatsOutput("  \n", `{"transports":{}}`))
	assert.Equal(t, "", selectStatsOutput("", ""))
}
