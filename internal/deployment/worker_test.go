...
func TestGetWorkerDeploymentConditionScaling(t *testing.T) {
	...
	t.Run("no queue transports keeps running with queue error message", func(t *testing.T) {
		emptyStore := store
		emptyStore.Status.QueueState = v1.QueueCondition{Error: "empty messenger:stats output"}
		c := fake.NewClientBuilder().WithScheme(scheme).Build()

		con := deployment.GetWorkerDeploymentCondition(context.Background(), emptyStore, c, true)
		assert.Equal(t, v1.DeploymentStateRunning, con.State)
		assert.Contains(t, con.Message, "empty messenger:stats output")
		assert.Equal(t, "0/0", con.Ready)
	})
...
func TestWorkerDeployment(t *testing.T) {
	...
	t.Run("test container merge spec", func(t *testing.T) {
		...
		assert.Equal(t, "worker-volume", container.VolumeMounts[1].Name)
		assert.Equal(t, "/worker", container.VolumeMounts[1].MountPath)
