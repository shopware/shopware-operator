...
func (q *QueueStats) logger() *zap.SugaredLogger {
	if q.Logger != nil {
		return q.Logger
	}
...
func (q *QueueStats) transports(ctx context.Context, store *v1.Store) []v1.QueueTransportStats {
	...
	if err != nil {
		q.logger().Warnw("failed to get live queue stats from admin pod, falling back to last reconciled state",
			zap.String("store", store.Name),
			zap.String("namespace", store.Namespace),
			zap.Error(err))
