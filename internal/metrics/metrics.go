...
var (
	...
	storeScheduledTaskSuspended = prometheus.NewGaugeVec(prometheus.GaugeOpts{
		...
		Help: "Whether the scheduled task CronJob is suspended (1) or active/not found (0)",
	}, []string{"store", "namespace"})
...
func setDeploymentMetrics(name, ns, deploymentType string, cond v1.DeploymentCondition) {
	// Parse available from the Ready field (format: "available/desired")
	var available int
	if cond.Ready != "" {
		fmtScan(cond.Ready, &available, new(int))
	}
