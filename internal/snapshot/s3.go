...
func (s *SnapshotService) uploadToS3(ctx context.Context, cfg *config.SnapshotConfig, s3Url string, contentType string, reader io.ReadCloser) error {
	defer reader.Close()
	endpoint, bucket, objectFile, err := parseS3URL(s3Url)
	if err != nil {
	...
	logging.FromContext(ctx).Debugw("Init minio client", zap.String("endpoint", endpoint), zap.String("bucket", bucket), zap.String("objectFile", objectFile))
...
func (s *SnapshotService) downloadFromS3(ctx context.Context, cfg *config.SnapshotConfig, s3Url string) (io.ReadCloser, error) {
	...
	logging.FromContext(ctx).Debugw("Init minio client for download", zap.String("endpoint", endpoint), zap.String("bucket", bucket), zap.String("objectFile", objectFile))
